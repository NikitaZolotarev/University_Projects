...
def get_key():
    with open('/Users/nikitazolotarev/Downloads/crypto/AES_key.txt','rb') as f:
        key = f.read()
    return key
...
while True:
    ...
    if message:
        ...
        # шифруем
        encrypted_message = cipher_aes.encrypt(pad(message.encode('utf-8'), BLOCK_SIZE))
...
#проверить recv
         ...
         #   message.replace(message)
            #дешифруем
            #decrypted_message = cipher_aes.decrypt(message)
            decrypted_message = unpad(cipher_aes.decrypt(message), BLOCK_SIZE).decode('utf-8')
